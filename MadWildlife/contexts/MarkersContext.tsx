...
// Initial Wildlife Data
const initialWildlifeData: WildlifeItem[] = [
  // Fauna
  {
    id: "fauna-1",
    name: "Red Fox",
...
// Helper function to convert wildlife items to map markers
const convertToMarkers = (data: WildlifeItem[]): MarkerWithMetadata[] => {
  return data
...
export const MarkersProvider = ({ children }: { children: ReactNode }) => {
  const [wildlifeData, setWildlifeData] = useState<WildlifeItem[]>(initialWildlifeData);
  const [markers, setMarkers] = useState<MarkerWithMetadata[]>(convertToMarkers(initialWildlifeData));

  const addMarker = (marker: MarkerWithMetadata) => {
